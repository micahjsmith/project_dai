...
lint: ## check style with flake8 and isort
	flake8 project_dai tests
	isort -c --recursive project_dai tests
...
fix-lint: ## fix lint issues using autoflake, autopep8, and isort
	find project_dai -name '*.py' | xargs autoflake --in-place --remove-all-unused-imports --remove-unused-variables
	autopep8 --in-place --recursive --aggressive project_dai
	isort --apply --atomic --recursive project_dai
...
coverage: ## check code coverage quickly with the default Python
	coverage run --source project_dai -m pytest
	coverage report -m
	coverage html
...
docs: clean-docs ## generate Sphinx HTML documentation, including API docs
	sphinx-apidoc --module-first --separate -o docs/api/ project_dai
	$(MAKE) -C docs html
